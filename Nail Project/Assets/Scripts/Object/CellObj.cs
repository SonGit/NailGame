...
public class CellObj : MonoBehaviour
{
    ...
    {
		//if cellType is above 9,it means the cell has item under it 
		if (cell.CellType > 9) {
			...
			PowerItemManager.Instance.CheckPowerItemForCompletion(cell);

			//bool val = GribManager.cell.CheckCellsForItemCompletion_2x2(cell);

			//if(val)
			//{
			// GribManager.cell.GiveItem(cell);
			//}

			UnlockItemSprite (cell.CellType - 10);
...
//			print ("UNLOCKED: " + val);
	...
	/// </summary>
	/// <param name="type"></param>
	public void UnlockItemSprite(int type)
	{
		this.GetComponent<SpriteRenderer>().sprite = GribManager.cell.UnlockedItemSprite[type];
	}
    ...
    void setChilEffectSprite(int celleffect)
    {
        if (celleffect > 0)
        {
			transform.GetChild(0).GetComponent<SpriteRenderer>().sprite = GribManager.cell.CellEffectSprite[celleffect];
        }
        else
        {
            transform.GetChild(0).gameObject.SetActive(false);

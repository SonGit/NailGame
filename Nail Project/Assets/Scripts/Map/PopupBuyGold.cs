...
public class PopupBuyGold : MonoBehaviour {
	
	private static GameObject _instance = null;
	private static bool IsPopupShow = false;

	public GameObject _GoldBarItem;
	
	public static void ShowPopup() {
		if (_instance == null) {
			GameObject mapLayer =  GameObject.Find("Canvas");
			_instance = Instantiate( Resources.Load("Prefabs/Popup/PopupBuyGold") as GameObject );
		...
		if (IsPopupShow) {
			...
			_instance.gameObject.GetComponent<PopupBuyGold> ().CloseButtonHandle();
		}
	}
